...
async function addTarget(config, { name, imageBuffer, videoBuffer }) {
  ...
  const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "webar-add-"));
  try {
    const newImagePath = path.join(tmpDir, `${id}.png`);
    fs.writeFileSync(newImagePath, imageBuffer);

    const imagePaths = [];
    for (let i = 0; i < config.targets.length; i++) {
      const ext = config.targets[i].targetImage.includes(".png") ? ".png" : ".jpg";
      const p = path.join(tmpDir, `existing-${i}${ext}`);
      await downloadToFile(config.targets[i].targetImage, p);
      imagePaths.push(p);
    }
    ...
    const [imageResult, videoResult, mindResult] = await Promise.all([
      cloudinary.uploadImage(imageBuffer, id),
      cloudinary.uploadVideo(videoBuffer, id),
      cloudinary.uploadMind(fs.readFileSync(mindPath), "targets"),
    ]);
